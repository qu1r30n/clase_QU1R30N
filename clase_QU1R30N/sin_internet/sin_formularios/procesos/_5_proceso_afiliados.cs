...
namespace clase_QU1R30N.sin_internet.sin_formularios.procesos
{
    internal class _5_proceso_afiliados
    {
        ...
        public string registro_unificado_cod3_r_(string dir_arch_afiliados, string dir_arch_niveles, string id_enc, string tabla_enc, string datos, object caracter_separacion_obj = null)
        {
            ...
            string[] res_esp_NIV_enc_unificado = resultado_inf_direccion_tab_NIVELES_IDHORIZONTAL_enc_unificado.Split(G_caracter_para_confirmacion_o_error[0][0]);


            if (res_esp_enc_unificado[0] == "1" && res_esp_NIV_enc_unificado[0] == "1")
            {

                int indic_afil_unificado = Convert.ToInt32(res_esp_enc_unificado[1]);
                int indic_niv_afil_unificado = Convert.ToInt32(res_esp_NIV_enc_unificado[1]);
                //0_id_usuario|1_id_patrocinador|2_tabla_patrocinador|3_id_encargado|5_tabla_encargado|5_diner|6_a_pagar|7_datos|8_encargados|
                ...
                string nivel_encargados = "";
                string nivel_usuario = "";
                for (int i = G_donde_inicia_la_tabla; i < Tex_base.GG_base_arreglo_de_arreglos[indic_afil_unificado].Length; i++)
                {
                    string[] info_base_split = Tex_base.GG_base_arreglo_de_arreglos[indic_afil_unificado][i].Split(caracter_separacion_string[0][0]);
                    //en registro simple en la base el id_encargado el el id_patrocinador
                    if (info_base_split[0] == id_enc)
                    {
                        nivel_encargados = info_base_split[5];
                        nivel_usuario = "" + (Convert.ToInt32(nivel_encargados) + 1);
                        break;
                    }
                }
                ...
                bool encontro_nivel_usuario = false;
                string id_horizontal_usuario_simple = "";
                for (int j = G_donde_inicia_la_tabla; j < Tex_base.GG_base_arreglo_de_arreglos[indic_niv_afil_unificado].Length; j++)
                {

                    string[] info_base_split = Tex_base.GG_base_arreglo_de_arreglos[indic_niv_afil_unificado][j].Split(caracter_separacion_string[0][0]);
                    ...
                    if (info_base_split[0] == nivel_usuario)
                    {
                        ...
                        Tex_base.GG_base_arreglo_de_arreglos[indic_niv_afil_unificado][j] = string.Join(caracter_separacion_string[0], info_base_split);
                        info_a_retornar = Tex_base.GG_base_arreglo_de_arreglos[indic_niv_afil_unificado][j];
                        bas.cambiar_archivo_con_arreglo(direccion_tab_NIVELES_IDHORIZONTAL_enc_unificado, Tex_base.GG_base_arreglo_de_arreglos[indic_niv_afil_unificado]);
                        break;
                    }
                ...
                {
                    id_horizontal_usuario_simple = "1";
                    info_a_retornar = bas.Agregar(direccion_tab_NIVELES_IDHORIZONTAL_enc_unificado, nivel_usuario + caracter_separacion_string[0] + id_horizontal_usuario_simple + caracter_separacion_string[0]);
            ...
            else
            {
                ...
                {
                    info_a_retornar = resultado_inf_enc_unificado;
                }
                else if (res_esp_NIV_enc_unificado[0] != "1")
                {
                    info_a_retornar = resultado_inf_direccion_tab_NIVELES_IDHORIZONTAL_enc_unificado;
                }
                else
                {
                    return null;
